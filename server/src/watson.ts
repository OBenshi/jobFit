...
const watsonTA = async (text: string): Promise<any> => {
  ...
  const benshiArr = response.result.document_tone.tones.reduce(
    (acc, cur) => ({ ...acc, [cur.tone_name]: cur.score }),
    {}
  );
  console.log(`benshiArr`, benshiArr);
  //   console.log("response from TAn ", JSON.stringify(response.result, null, 2));
  let newArr: string[] = response.result.document_tone.tones.map(
    (tone) => tone.tone_id
  );
